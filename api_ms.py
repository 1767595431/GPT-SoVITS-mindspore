...
import os, re, json, logging, shutil, time, sys, threading, configparser, datetime
...
def _apply_cli_overrides():
    ...
    - --root-path <path>               外网代理路径前缀（如：/ttsHttp），仅用于生成公网资源 URL
    - --base-url <url>                 外网基础 URL（如：https://36.136.54.165/media）
    """
    try:
        argv = sys.argv[1:]
...
#   A) proxy_pass http://IP:9881/;     → 剥前缀，后端收到 /index
#   B) proxy_pass http://IP:9881;      → 不剥前缀，后端收到 /media/ttsHttp/index
...
def _merge_port_into_base_url(base: str, request: Request) -> str:
    """仅 HTTP 非标准端口时补端口；HTTPS 不追加 nginx 内部端口。"""
...
def _internal_base_url(request: Optional[Request] = None) -> str:
    """未同时配置 base-url + root-path 时：http://IP:端口。"""
...
def _model_for_store(item: dict) -> dict:
    return {k: v for k, v in item.items() if k not in ("avatar_url", "refer_wav_url")}
...
async def strip_proxy_prefix_middleware(request: Request, call_next):
    """兼容 nginx proxy_pass 带/不带末尾斜杠两种转发方式。"""
...
# JSON 文件读写锁（fcntl 仅 Linux；Windows 用 threading.Lock）
try:
...
def _write_json_file(path: Path, items: list) -> None:
...
def _read_store() -> list:
    items = _read_json_file(STORE_PATH)
...
def _write_store(items: list) -> None:
    ...
    _write_json_file(STORE_PATH, cleaned)


def _read_categories() -> list:
    return _read_json_file(VOICE_CATEGORY_STORE)


def _write_categories(items: list) -> None:
    _write_json_file(VOICE_CATEGORY_STORE, items)


def _read_task_store() -> list:
    return _read_json_file(VOICE_TASK_STORE)


def _write_task_store(items: list) -> None:
    _write_json_file(VOICE_TASK_STORE, items)
...
async def healthz(request: Request):
    ...
    return {"status": "ok", "index_url": f"{base}/"}
